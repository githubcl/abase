...
public class ImageUtil {
	...
	public static Bitmap getBitmap(String image) {
		...
		opts.inSampleSize = 2;
		return BitmapFactory.decodeFile(image, opts);
	}
	 ...
	 *            图片大小最大值，单位k
	 */
	public static void compress(String image, int max) {
		try {
			...
			while (baos.toByteArray().length / 1024 > max) {
				...
				options -= 10;// 每次都减少10
				LogUtil.debug("图片大小：" + (baos.toByteArray().length / 1024));
			}
			FileOutputStream b = null;

			b = new FileOutputStream(image);
			bitmap.compress(Bitmap.CompressFormat.JPEG, options, b);// 把数据写入文件
			b.flush();
			b.close();
	 ...
	 *            图片大小最大值，单位k
	 */
	public static void compress(String image, int min, int max) {
		try {
			Bitmap bitmap = getBitmap(image);
			ByteArrayOutputStream baos = new ByteArrayOutputStream();
			bitmap.compress(Bitmap.CompressFormat.JPEG, 100, baos);// 质量压缩方法，这里100表示不压缩，把压缩后的数据存放到baos中
			...
			while (len / 1024 > max || len / 1024 < min) {
				// 循环判断如果压缩后图片是否大于100kb,大于继续压缩
				baos.reset();// 重置baos即清空baos
				bitmap.compress(Bitmap.CompressFormat.JPEG, options, baos);// 这里压缩options%，把压缩后的数据存放到baos中
				if (len / 1024 > max) {
	...
	public static String encodeBase64Image(String image) {
		String value = null;
		try {
			...
			value = new String(Base64.encode(baos.toByteArray())); // 进行Base64编码
			fis.close();
			baos.close();
		} catch (Exception e) {
			LogUtil.error("编码图片出现异常...");
			e.printStackTrace();
		}
		return value;
	}
		...
		if (bitmap != null) {
			try {
				if (!bitmap.isRecycled()) {
					LogUtil.debug("Bitmap释放" + bitmap.toString());
					bitmap.recycle();
				}
			} catch (Exception e) {
			}
